  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      try {
        ...
        setTotalPages(Math.ceil((res.total ?? res.length) / PAGE_SIZE));
      } catch {
    ...
    <div className="max-w-5xl mx-auto px-4 py-12">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        ...
        <select
          ...
          className="border border-gray-300 rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#1f4c7a] w-full sm:w-[180px]"
        >
          {newsTypeOptions.map((o) => (
            ...
            >
              ...
              <div className="flex-shrink-0 w-12 h-12 rounded-lg bg-[#e8f0f9] flex items-center justify-center text-2xl">
                {item.newsType === "Event"
                  ...
                  : item.newsType === "Announcement"
                    ? "📢"
                    : item.newsType === "Education"
                      ? "🎓"
                      : item.newsType === "Admission"
                        ? "✏️"
                        : "📰"}
              </div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  <span
                    className={`text-xs font-medium px-2 py-0.5 rounded-full ${newsTypeColor[item.newsType] || "bg-gray-100 text-gray-600"}`}
                  >
